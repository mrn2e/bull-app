import{D as f,i as m,b as x,I as y,a as E}from"./c3fc54f1.js";/**
 ...
 */class w extends HTMLElement{connectedCallback(){const t=this.getAttribute("type")||"",r=this.getAttribute("opponent")||"";this.innerHTML=`
    ...
    `,this.querySelector(".event").addEventListener("click",()=>{this.dispatchEvent(new CustomEvent("event-clicked",{bubbles:!0}))})}}customElements.define("bull-calendar-events",w);class k extends HTMLElement{connectedCallback(){this.innerHTML=`
      ...
      <div id="calendar"></div>
    `,this.loadEvents()}async loadEvents(){try{const t=await(await fetch("./bull-calendar-events-data.json")).json();this.renderCalendar(t)}catch(t){console.error("Error loading events:",t),this.renderCalendar([])}}renderCalendar(t){const r=this.querySelector("#calendar"),a=2026,s=3,v=new Date(a,s,1),c=new Date(a,s+1,0).getDate(),i=v.getDay();let n="<table><thead><tr>";["Sun","Mon","Tue","Wed","Thu","Fri","Sat"].forEach(e=>n+=`<th>${e}</th>`),n+="</tr></thead><tbody><tr>";for(let e=0;e<i;e++)n+="<td></td>";for(let e=1;e<=c;e++){(i+e-1)%7===0&&e!==1&&(n+="</tr><tr>");const h=`${a}-${String(s+1).padStart(2,"0")}-${String(e).padStart(2,"0")}`,l=t.find(g=>g.date===h),b=l?`<bull-calendar-events type="${l.type}" opponent="${l.opponent}"></bull-calendar-events>`:"";n+=`<td><span class="day">${e}</span>${b}</td>`}const u=(7-(i+c)%7)%7;for(let e=0;e<u;e++)n+="<td></td>";n+="</tr></tbody></table>";let d='<div id="events-list"><h3>Events</h3><div class="events-container">';t.forEach(e=>{d+=`<div class="event-item" onclick="this.dispatchEvent(new CustomEvent('event-clicked', {bubbles: true}))">${e.date}: ${e.type} vs ${e.opponent}</div>`}),d+="</div></div>",n+=d,r.innerHTML=n}}customElements.define("bull-calendar",k);function $(o){return o==="./lib/bull-app.haxProperties.json"?new URL(new URL("ad52f369.json",import.meta.url).href):new Promise(function(t,r){(typeof queueMicrotask=="function"?queueMicrotask:setTimeout)(r.bind(null,new Error("Unknown variable dynamic new URL statement: "+o)))})}class p extends f(y(E)){static get tag(){return"bull-roster"}constructor(){super()}static get properties(){return{...super.properties,title:{type:String}}}static get styles(){return[super.styles,m`:host{display:block;color:var(--ddd-theme-primary);background-color:var(--ddd-theme-accent);font-family:var(--ddd-font-navigation)}.wrapper{margin:var(--ddd-spacing-2);padding:var(--ddd-spacing-4)}h3 span{font-size:var(--bull-app-label-font-size,var(--ddd-font-size-s))}`]}render(){return x` <div class="wrapper"> <slot></slot> </div>`}static get haxProperties(){return $(`./lib/${this.tag}.haxProperties.json`).href}}globalThis.customElements.define(p.tag,p);/**
 * Copyright 2026 Mayita
 * @license Apache-2.0, see LICENSE for full text.
